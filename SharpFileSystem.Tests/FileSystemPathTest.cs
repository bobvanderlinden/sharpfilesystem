...
namespace SharpFileSystem.Tests
{
    ...
    {
        private static readonly FilePath directoryA = FilePath.Parse("/directorya/");

        private static FilePath directoryB = FilePath.Parse("/directorya/directoryb/");

        private static FilePath fileA = FilePath.Parse("/filea");

        private static FilePath fileB = FilePath.Parse("/directorya/fileb.txt");

        private static FilePath root = FilePath.Root;

        private FilePath[] _paths = new[]
                                                                                            {
                                                        root,
                                                        directoryA,
                                                        fileA,
        ...
        private FilePath fileC;
        public FileSystemPathTest()
        {
        }

        private IEnumerable<FilePath> Directories { get { return _paths.Where(p => p.IsDirectory); } }

        private IEnumerable<FilePath> Files { get { return _paths.Where(p => p.IsFile); } }
        ...
        public void AppendPathTest()
        {
            ...
            var subpath = FilePath.Parse("/dir/file");
        ...
        public void GetExtensionTest()
        {
            ...
            fileC = FilePath.Parse("/directory.txt/filec");
        ...
        {
            Assert.IsTrue(FilePath.IsRooted("/filea"));
            Assert.IsTrue(FilePath.IsRooted("/directorya/"));
            Assert.IsFalse(FilePath.IsRooted("filea"));
            Assert.IsFalse(FilePath.IsRooted("directorya/"));
            Assert.IsTrue(_paths.All(p => FilePath.IsRooted(p.ToString())));
        }
        ...
        {
            Assert.IsTrue(_paths.All(p => p == FilePath.Parse(p.ToString())));
            EAssert.Throws<ArgumentNullException>(() => FilePath.Parse(null));
            EAssert.Throws<ParseException>(() => FilePath.Parse("thisisnotapath"));
            EAssert.Throws<ParseException>(() => FilePath.Parse("/thisisainvalid//path"));
        }
        ...
        {
            Assert.AreEqual(fileB.RemoveChild(FilePath.Parse("/fileb.txt")), directoryA);
            Assert.AreEqual(directoryB.RemoveChild(FilePath.Parse("/directoryb/")), directoryA);
            Assert.AreEqual(directoryB.RemoveChild(directoryB), root);
            Assert.AreEqual(fileB.RemoveChild(fileB), root);
            EAssert.Throws<ArgumentException>(() => directoryA.RemoveChild(FilePath.Parse("/nonexistantchild")));
            EAssert.Throws<ArgumentException>(() => directoryA.RemoveChild(FilePath.Parse("/directorya")));
        }
        ...
        public void RemoveParentTest()
        {
            ...
            Assert.AreEqual(fileB.RemoveParent(directoryA), FilePath.Parse("/fileb.txt"));
            Assert.AreEqual(root.RemoveParent(root), root);
            Assert.AreEqual(directoryB.RemoveParent(root), directoryB);
            EAssert.Throws<ArgumentException>(() => fileB.RemoveParent(FilePath.Parse("/nonexistantparent/")));
            EAssert.Throws<ArgumentException>(() => fileB.RemoveParent(FilePath.Parse("/nonexistantparent")));
            EAssert.Throws<ArgumentException>(() => fileB.RemoveParent(FilePath.Parse("/fileb.txt")));
            EAssert.Throws<ArgumentException>(() => fileB.RemoveParent(FilePath.Parse("/directorya")));
        }
        ...
        {
            Assert.AreEqual(FilePath.Parse("/"), root);
        }
        ...
        {
            string s = "/directorya/";
            Assert.AreEqual(s, FilePath.Parse(s).ToString());
        }
    }
}
